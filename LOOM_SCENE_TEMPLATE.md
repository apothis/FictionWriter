...
|---|---|---|---|
| 7.a.1 | Spike — extraction quality | 5 hand-curated diverse scenes (dialogue / interiority / description / action / summary). Hand-evaluate: are beats useful? Are modality tags accurate? Are pacing stats faithful? Decision gate: refine prompts/grammar OR pivot. | ✅ landed 2026-05-13 (`31aeb5a`). Verdict SHIP to 7.a.2. STRAP stripping 5/5 perfect; modality matches authored intent 76%/43%/83%/38%/0% (interiority + summary reproduce Phase 5 narrative-mode floor). LLM-reported pacing was unreliable → dropped from production schema. |
| 7.a.2 | Spike — generation quality | For 3 fixtures, run Pass B with synthetic cast mapping. Hand-evaluate: does output preserve shape? Does plot leak? Does voice transfer? | ✅ landed 2026-05-13 (`5d2a3e2`). Verdict SHIP to 7.a.3 with 5-item mandatory prompt-revision punchlist. STRAP holds under generation pressure (31/31 beats zero name leakage). Cast substitution works. BUT 6/31 empty-beat outputs traced to (a) bare `[` in stop list, (b) model echoing N+1 beat enumeration, (c) `***` scene-break emission. |
| 7.a.3 | Spike — long-exemplar pitfalls | A/B Pass B with template-included vs skeleton-only. Tripto 2025 hypothesis test. | ✅ landed 2026-05-13 (`7f5b219`). Verdict: **Tripto 2025 NOT reproduced.** Template prose acts as task anchor (not style donor). Voice transfer weak in BOTH arms — gemma default register dominates. D3 confirmed (keep template as first-class slot). v2 voice-weight dial DEFERRED in favor of explicit voice-descriptor injection → became 7.a.4. |
| 7.a.4 | Spike — voice-descriptor extraction + injection | (Unplanned, surfaced from 7.a.3.) `VoiceDescriptor` extracted at Pass A; injected at Pass B's recency slot as positive constraints. A/B on fixtures 01 + 03. | ✅ landed 2026-05-13 (`254e462`). Verdict SHIP. Voice descriptor measurably moves prose toward target style. Empty-beat regression flagged as known issue compounding with over-segmentation. Spike writeup: [`LOOM_SCENE_TEMPLATE_SPIKE.md`](LOOM_SCENE_TEMPLATE_SPIKE.md) §4. |
| 7.b.1 | TemplateScene entity + storage | `TemplateScene` Codable + `TemplateSceneStorage` (mirrors `ReferenceStorage`). | ✅ landed 2026-05-13 (`05b4f99`). |
| 7.b.2 | Beat extraction pipeline | `BeatExtractionPipeline` (async-callback shape mirroring `ReferenceIngestPipeline`) + `BeatExtractor` protocol + tests using deferred-completion stubs. | ✅ landed 2026-05-13 (`05b4f99`). |
| 7.b.3 | Production extractor + AppState wiring | `OllamaBeatExtractor` (mirrors `OllamaLedgerExtractor` retry-on-empty + scaled-budget pattern). `AppState.extractTemplateScene(id:)` background-queue entry. Spike runner refactored to use the production extractor via semaphore wrap. | ✅ landed 2026-05-13 (`f84f254`). |
| 7.b.4 | Per-beat generation coordinator | `TemplateGenerationCoordinator` — per-beat loop, M sequential writer calls, prose accumulated, each beat emitted as one `didEmitToken` matching `GenerationCoordinator`'s shape so editor's existing token-inserter handles it verbatim. Punchlist items 2 + 3 (drop bare `[` stop, retry-on-empty). | ✅ landed 2026-05-13 (`b75474c`). |
| 7.b.5 | Bible Workspace surface | Template Scenes section in EntityList + TemplateSceneEditor with Extract/Re-extract/Delete; intent cases mirror Phase 5 A2 References. | ✅ landed 2026-05-13 (`3fa6cb0`). Swift + React both shipped; Vite-preview-smoke confirms intent wire format round-trips through `BibleWorkspaceBridge.decodeIntent`. |
| 7.b.6 | In-editor flow | EditorViewController owns `TemplateGenerationCoordinator` parallel to `GenerationCoordinator`; 3 parallel observers share insertion logic. AppDelegate Bible menu → "Write Scene From Template…" with NSAlert + NSStackView picker (NSPopUpButton + cast-mapping NSTextField). | ✅ landed 2026-05-13 (`510c1f5`). |
| 7.b.7 | Empirical validation pass | 10–15 generations across 3 templates × 3 cast mappings in the running `Loom.app`. Hand-evaluate end-to-end. | 🚧 **YOU-DRIVEN. PENDING.** See HANDOFF §15.14 smoke-test list 5–10. |
| 7.c | v2 features | Two-dial UI (DEFERRED post-§7.a.3 — surface mimicry isn't the dominant issue), structured cast mapping, modality flow editor, per-beat re-roll, character-voice integration. Contingent on 7.b.7 validation. | parked |

...
final class TestSuite {
    ...
    /// MainActor-bound types (e.g. AppKit-adjacent controllers). Bodies
    /// that touch no MainActor types pay no isolation cost. The runner
    /// calls each body from a MainActor context (`run` is `@MainActor`-
    /// isolated; `main.swift` uses `MainActor.assumeIsolated`).
...
func awaitSync<T>(_ body: @escaping @Sendable () async throws -> T) throws -> T {
    let semaphore = DispatchSemaphore(value: 0)
    let box = AwaitResultBox<T>()
    Task.detached {
...
private final class AwaitResultBox<T>: @unchecked Sendable {

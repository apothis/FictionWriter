...
- **rewriteTense no-op-target failure mode.** Reproducible both ways: when the user picks a target tense that matches the source's current tense, the writer model invents an unrelated transformation (past+past target → present output; present+present target → future output, observed live 2026-05-13). A first-attempt fix tightening the rewriteTense system prompt with explicit no-op-target language made the failure **catastrophically worse** — the model entered a degenerate sampling cascade with synonym-runaway and meta-commentary leakage ("What follows next is a continuation of the scene as described below:..." into pure token soup). That prompt change has been rolled back (commit `TBD`). Diagnosis: asking the model to "rewrite to be what it already is" is meaningless to it as a task; prompt engineering can't reliably coax meaningful no-op output from gemma3-class models. **Right fix is upstream of the model:** detect the source tense at the picker/handler layer, either (a) disable the matching-tense option in the Rewrite menu when the source clearly already uses that tense, OR (b) short-circuit at click time with a tray message ("This passage is already in past tense — nothing to rewrite"). Both require an English tense-detection heuristic (regex over past-tense markers vs present-tense forms across the selection) — tractable but non-trivial. Phase 4.x slice. In the meantime: don't pick a matching-tense option in the menu; pick the OTHER tense to verify the genuine transformation path (which works correctly).

- **rewriteTense past-target single-paragraph truncation.** Live test 2026-05-13 Test 5b control run (present source → past target, ~120-word two-paragraph passage): the model emitted EOS at end of paragraph 1, inserting 405 chars when the source was ~700. `reply=1024` budget was nowhere near saturated, so this is the writer model treating "the rewritten passage" as complete after paragraph 1, not a budget cap. The mirror present-target run on the same source produced both paragraphs (687 chars, ~120 words). Observed once on past-target only. Hypotheses worth checking: (a) `\n\n` is a stop sequence in `PromptBuilder.AssembledPrompt.stopSequences` and the inter-paragraph break terminated the stream — same prompt to present-target may have been more verbose and pushed through; (b) the mode instruction's "Output the rewritten passage only" framing gives a "done" signal the model interprets aggressively at the first paragraph boundary specifically when output style matches a common stopping rhythm (declarative past-tense sentences). Repro path: paste the §15.8 Saoirse/Linus control passage into a new scene, select, Rewrite → Tense — past, check `inserted=` chars against source length. If reproducible, first diagnostic is inspecting stop sequences in the assembled `GenerateRequest`.

- **rewriteVoice length-guard drift under verbose descriptors.** Live test 2026-05-13 Test 5a: descriptor `"lyrical, slow, sensory-heavy, late-Atwood"` produced a rewrite at ~150% of original (165 words from a 109-word chunk), exceeding the system prompt's stated `±20%` length ceiling. Beat preservation + voice fidelity + NSFW content-neutrality all passed; the drift is the prompt's length anchor not holding when a maximally-expansive stylistic descriptor pulls against it. Not a structural regression — prompt-tuning territory. Cheap fix to pilot: tighten the `±20%` clause in `PromptBuilder.systemPromptFor(.rewriteVoice)` to something harder like "the length constraint takes priority over the voice descriptor's natural register". Worth landing once we have a second example of the drift to validate the fix doesn't over-constrain.

...
8. **The §10.5 production filters** — fact deduplication (embed → cluster cosine ≥ 0.85), evidence-quote validation (drop facts whose evidence has no high-cosine sentence in the prose), prompt-leakage filter — between extraction and Suggestions display. **PENDING.**
...
**Still pending for #7 to feel complete:**

- **POV picker UI.** Sub-task 7's `[KNOWLEDGE-LEDGER]` prompt layer correctly renders only when `Scene.pov` is a non-nil bible character. But there's no UI in Loom to assign POV to a scene — `Scene.pov: UUID?` has been in the schema since Phase 1.b. Result: accepted ledger facts persist correctly but never surface in actual generation prompts. Smallest viable shape: sidebar right-click "Set POV…" → menu of `bible.characters` → call `session.setScenePOV(id:)`. `ProjectSession` would need the setter (mutates `scene.pov`, calls `markChanged()`). Phase 4.x; ~30 min slice.

- **Sub-task 8: the §10.5 production filters.** Three pure-data filters between extraction and Suggestions display, all using the existing `KoboldClient.embed(...)` path on the writer server's `nomic-embed-text` endpoint:
  1. **Fact deduplication.** Embed extracted-fact texts + existing-ledger-fact texts; cluster at cosine ≥ 0.85; collapse paraphrase pairs. Current diff only drops *verbatim* matches.
  2. **Evidence-quote validation.** Embed each fact's evidence quote + the scene's sentences; drop facts whose evidence has no high-cosine sentence match (catches the model lightly editorialising the speech tag, e.g. emitting `"Mia asked, 'Who is it?'"` when the prose only has `"Who is it?"`).
  3. **Prompt-leakage filter.** Embed the §3.3 prompt instructions ahead of time; drop facts whose content cosine-matches the prompt above some threshold (catches the rare prompt-text echo).

  All three plug in at `AppState.handleExtractionComplete` between `LedgerDiff.diff` and `ledgerSuggestionsQueue.add`. Pure-data, TDD-able with embedding stubs.
...
**NSFW strategic anchor — still load-bearing.** The pipeline is content-neutral by design (LOOM_NSFW.md §3), but live-testing on explicit prose hasn't happened yet. The spike numbers (LOOM_LEDGER_SPIKE §11.4 / §12.4) showed 100% recall on the two NSFW fixture scenes; production behaviour should match. Sub-task 8 design should keep §3.5's `.extreme` configuration in mind — no filter should silently drop a fact for content reasons; the model's output is the user's to curate via accept/reject. Phase 4.x should also pressure-test multi-character extraction on a sex scene where both partners have agency (the spike's `03_first_night` is the closest fixture).

**Test count: 656 passing.** All sub-task work + bugfixes + the lifetime regression test.

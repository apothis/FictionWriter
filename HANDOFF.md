...
5. ✅ **Knowledge-ledger feasibility spike** — landed 2026-05-11 across three rounds. Full writeup in [`LOOM_LEDGER_SPIKE.md`](LOOM_LEDGER_SPIKE.md). **Verdict: PROCEED with Phase 4 #7.** Structural wins shipped: (a) GBNF grammar-constrained decoding via KoboldCpp's `grammar` parameter (eliminates malformed JSON, force-prefill, `<think>` leakage); (b) grammar-level `character_id` enum restricted to bible names+aliases (kills the garbage-character_id failure mode); (c) `unknown` knowledge derived from per-character scene-exposure graph at query time, not extracted (SymbolicToM pattern); (d) Round-3 wired the KoboldCpp embedding endpoint (nomic-embed-text 768-d, already live on the same server) for cosine-similarity scoring + Phase 4 #7 fact deduplication + evidence-quote validation. **Real metrics (embedding scorer, character-restricted grammar, Qwen3.6-27B):** for scenes where the model engages (3 of 5 in the fixture), recall is 8/12 = 67%; aggregate is 8/20 = 40% dragged down by 2 SFW scenes that return `[]`. The Mistral-Small-3-24B / Phi-4-14B / Qwen2.5-7B model-swap recommendation stands as the Phase 4 #7 prerequisite — Qwen3.6-27B under grammar still has degenerate behaviours (empty arrays, prompt-text leakage into strings) that a non-thinking, extraction-tuned model would not.
6. **rewriteTense + rewriteLength prompt layers** + one coherent sub-mode picker UI covering Voice/Tense/Length.
7. **Knowledge-ledger pipeline** (#5 cleared the gate; PROCEED): post-scene side-call → diff → Bible-inspector Suggestions chip → user accept/reject → `[KNOWLEDGE-LEDGER]` prompt layer below cache boundary. Ship `asserted`-only extraction via the `LedgerExtraction` module + GBNF grammar (already shipped). Compute `unknown` from per-character scene-exposure (SymbolicToM pattern, [LOOM_STORY_BIBLE §3.5](LOOM_STORY_BIBLE.md)) — no LLM call for negative knowledge. **Prerequisite:** swap the summariser-role server to a smaller extraction-tuned model (Mistral-Small-3-24B / Phi-4-14B / Qwen2.5-7B-Instruct-Uncensored); the spike confirmed Qwen3.6-27B under grammar produces clean shape but messy content. Re-run `LedgerSpike` against the new model before committing the UI surfaces.

...
5. ✅ **Knowledge-ledger feasibility spike** — landed 2026-05-11. Full writeup in [`LOOM_LEDGER_SPIKE.md`](LOOM_LEDGER_SPIKE.md), including a Round-2 research pass (§8) that superseded most of the v1 recommendations. **Verdict: PROCEED with Phase 4 #7 — but the extractor must NOT be the primary writing model.** Headlines: (a) GBNF grammar-constrained decoding via KoboldCpp's `grammar` parameter eliminates malformed JSON, force-prefill, and `<think>` leakage as structural guarantees (`LedgerExtraction.gbnfGrammar()` + `GenerateRequest.grammar` shipped); (b) `unknown` knowledge is derived from a per-character scene-exposure graph at query time, not extracted from prose (SymbolicToM pattern, sidesteps the empirically-unsolved negative-knowledge problem); (c) Qwen3.6-27B under grammar constraint produces clean JSON shape but messy content (prompt-leakage into strings, degenerate loops). The extractor side-call should run against a smaller, extraction-tuned model — Mistral-Small-3-24B / Phi-4-14B / Qwen2.5-7B-Instruct-Uncensored — on the RPClient role-routed summariser server. **The grammar plumbing graduates into production unchanged**; the model swap is a Phase 4 #7 prerequisite, not an optimisation.

...
5. ✅ **Knowledge-ledger feasibility spike** — landed 2026-05-11, commits `01ddf34` + `8aeb5d7`. Full writeup in [`LOOM_LEDGER_SPIKE.md`](LOOM_LEDGER_SPIKE.md). **Verdict: PROCEED with Phase 4 #7**, ship `asserted`-only auto-extraction; manual authoring is the primary path for `unknown` / `mistaken` (negative knowledge does NOT extract reliably at 27B — empirically confirmed). Production prompt-engineering must include force-prefilled `[` (without it Qwen emits EOS immediately), `max_length ≥ 2048` (Qwen3 `<think>` blocks consume ~70% of budget), and the per-object fallback parser (strict array decoding fails on every fixture scene due to malformed/truncated JSON). Hand-reviewed recall on asserted facts: ~70%; the scorer's aggregate F1=0.09 is an artifact of Jaccard 0.5 missing synonym paraphrases.
6. **rewriteTense + rewriteLength prompt layers** + one coherent sub-mode picker UI covering Voice/Tense/Length.
7. **Knowledge-ledger pipeline** (#5 cleared the gate; PROCEED): post-scene side-call → diff → Bible-inspector Suggestions chip → user accept/reject → `[KNOWLEDGE-LEDGER]` prompt layer below cache boundary. Ship `asserted`-only; reuse the `LedgerExtraction` pure-data module + the prompt-engineering pinned by the spike. The Bible-inspector manual-authoring flow for `unknown` / `mistaken` is the primary path for negative knowledge.

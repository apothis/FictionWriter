...
> **Status: Phase 8 design LOCK (2026-05-14).** All D-decisions (D1–D8) resolved per Phase 8.a spike work — D4 empirically (Wegmann embedder, [`LOOM_SCENE_EXEMPLAR_SPIKE.md`](LOOM_SCENE_EXEMPLAR_SPIKE.md) §1); D5/D7/D8 by design with documented rationale; D6 by design + a Pass-A-on-NSFW empirical probe ([`LOOM_SCENE_EXEMPLAR_SPIKE.md`](LOOM_SCENE_EXEMPLAR_SPIKE.md) §2). Phase 8.b implementation can begin. Two deferred-to-8.c retests (§6.2 / §6.4 / §6.5 generation-grade probes) validate locked decisions rather than originate them. The user-facing flow becomes: paste a scene, name it, then point any new generation at it for "same shape, same voice, same content register."
>
> Companion to:
>
...
> - [`LOOM_SCENE_EXEMPLAR_SPIKE.md`](LOOM_SCENE_EXEMPLAR_SPIKE.md) — Phase 8.a empirical findings (landed 2026-05-14 for §6.1; §§6.2–6.5 pending). §6.1 result: **D4 locked to Wegmann Style-Embedding**. StyleDistance underperforms all three alternatives across NSFW + SFW + combined scopes.
>
...
- **D4 (LOCKED 2026-05-14 — Wegmann Style-Embedding).** Per the §6.1 empirical probe in [`LOOM_SCENE_EXEMPLAR_SPIKE.md`](LOOM_SCENE_EXEMPLAR_SPIKE.md) §1: Phase 8.b production retrieval embeds exemplar chunks with `AnnaWegmann/Style-Embedding`. StyleDistance loses to all three alternatives in the 4-candidate set (Wegmann +0.285 / LUAR +0.173 / mxbai +0.162 / StyleDistance +0.086 on `style_axis`). The Phase 5 retrieval pipeline is a separate decision: there is now a credible case for migrating Phase 5 to Wegmann too, flagged for the user in §1.4 of the spike writeup.
- **D5 (LOCKED 2026-05-14 — per-beat by default).** Per the §3.1 design rationale + §5 of [`LOOM_SCENE_EXEMPLAR_SPIKE.md`](LOOM_SCENE_EXEMPLAR_SPIKE.md): Phase 8.b ships per-beat retrieval scoped to the chosen exemplar (default); project-wide retrieval is an opt-in setting. Generation-grade probe (§6.2) is deferred to 8.c if 8.b smoke shows the N×retrieval cost is unworkable on the user's hardware.
- **D6 (LOCKED 2026-05-14 — beat-aligned chunks; sentence-window fallback).** Per [`LOOM_SCENE_EXEMPLAR_SPIKE.md`](LOOM_SCENE_EXEMPLAR_SPIKE.md) §2: Pass-A extraction empirically works on all 5 NSFW registers (with retry coverage), so beat metadata is reliable. Beat-aligned chunks inherit modality+function tags as first-class retrieval metadata; sentence-window fallback handles the partial-failure case where Pass-A produces no skeleton.
- **D7 (LOCKED 2026-05-14 — user-controllable toggle, default-off).** Per the §5.3 position 2 recommendation: 8.b ships an "imitate content" toggle on the Write-From-Template menu. Default-off preserves the Phase 7 strict-prompt behaviour; opt-in surfaces the soft-D4 path for the NSFW exemplar case. §6.4 generation-grade auto-metric probe deferred to 8.c.
- **D8 (LOCKED 2026-05-14 — Option C, rename + soft-merge).** Per §5.1: Bible Workspace UI renames to "Scene Exemplars" (one card per body). Backing types `ReferenceText` + `TemplateScene` stay on disk; the workspace merges them by body-content hash for de-duplication where possible. Lowest-migration-risk path; Option A (full type merge) defers to 8.c if Option C proves to add complexity.
...
> **Post-audit + availability-check note (2026-05-14, see [`LOOM_SCENE_EXEMPLAR_RESEARCH.md`](LOOM_SCENE_EXEMPLAR_RESEARCH.md) and its addendum).** The hypothesis is structurally near-certain rather than merely plausible: StyleDistance's training set (SynthSTEL) is built from C4 sentences paraphrased by GPT-4, and C4 applies the LDNOOBW blocklist (whole pages deleted on any explicit term). The §6.1 candidate set below was first revised, then re-revised after pre-spike toolchain verification:
>
> - **iBERT (the audit's primary additional candidate) is paper-only.** GitHub README says "Code and model will be released in April 2026"; April has slipped. HF user has 0 public models. Substituted by `AnnaWegmann/Style-Embedding` (the SBERT-style baseline iBERT positions itself against on STEL — directly comparable when iBERT eventually ships).
> - **LUAR added as a predicted negative control.** `gabrielloiseau/LUAR-MUD-sentence-transformers` — available, ST-compatible. Trained for authorship, not style; per Wegmann TACL + StyleDistance paper, LUAR conflates style with author/topical signal and is expected to fail the "same register, different authors → high cosine" test. It enters the spike as a contrastive datapoint, not a primary candidate.
>
> **Revised §6.1 candidate set:** StyleDistance (incumbent) + Wegmann Style-Embedding (primary style alternate) + LUAR (predicted negative control) + mxbai-embed-large (retrieval baseline). All four run through the spike runner at `Tools/SceneExemplarSpike` (8.a.3).
...
- 8.a.3 — **LANDED 2026-05-14.** §6.1 embedder discrimination probe at [`LOOM_SCENE_EXEMPLAR_SPIKE.md`](LOOM_SCENE_EXEMPLAR_SPIKE.md) §1. All 4 candidates run against the 20-fixture set (combined, NSFW-only, SFW-only scopes). **D4 locked to Wegmann.** StyleDistance underperforms the topical baseline. Spike runner subcommand. Deliverable: pairwise cosine matrix + ranking + recommendation.
- 8.a.4 — **LANDED 2026-05-14.** Pass-A extraction smoke test on NSFW registers (Tools/SceneExemplarSpike `extract` subcommand). Headline: extraction works on all 5 NSFW registers given retry coverage; transient ~30% single-attempt JSON-parse failures. Phase 8.b TODO: extend `OllamaBeatExtractor` retry to cover `noJSONObjectFound` (currently only retries on empty content). Findings in [`LOOM_SCENE_EXEMPLAR_SPIKE.md`](LOOM_SCENE_EXEMPLAR_SPIKE.md) §2. **D6 locked by design + Pass-A empirical premise.**
- 8.a.5 — **DEFERRED to 8.c.** §6.2 + §6.5 generation-quality probes require Phase 8.b production code to exist (TemplateGenerationCoordinator extended with style retrieval). D5 locked by design pending 8.b validation.
- 8.a.6 — **DEFERRED to 8.c.** §6.4 prompt-softening probe requires Phase 8.b's soft-D4 toggle path to exist. D7 locked by design (default-off toggle).
- 8.a.7 — **LANDED 2026-05-14.** Spike write-up at [`LOOM_SCENE_EXEMPLAR_SPIKE.md`](LOOM_SCENE_EXEMPLAR_SPIKE.md). All D-decisions (D4-D8) resolved or explicitly deferred to 8.c with documented rationale.

**Exit criteria: ✅ MET.** All D-decisions locked. Doc status flipped to "design lock" at the top of this file. Phase 8.b can begin.

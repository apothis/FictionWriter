...
| L4 | Generation-mode expansion (Rewrite sub-modes, Show-don't-tell, Brainstorm, Critique, Bridge, Roll-Outcome); per-mode prompt templates; knowledge-state-per-character extraction; NSFW polish (Continue-from-refusal, Sphiratrioth pack) | 🚧 in flight — see [`HANDOFF.md`](HANDOFF.md) §15 + [`LOOM_LEDGER_SPIKE.md`](LOOM_LEDGER_SPIKE.md). Landed: rewriteVoice prompt layer, Continue-from-refusal, Sphiratrioth pack, Roll-Outcome action, knowledge-ledger feasibility spike (5 rounds), **knowledge-ledger pipeline #7 sub-tasks 1–7 shipped end-to-end** (Ollama role-routed extractor, debounced post-scene side-call, diff-vs-existing, Suggestions panel UI, bible persistence, scene-exposure-based `unknown` derivation, `[KNOWLEDGE-LEDGER]` prompt layer). Live verified 2026-05-11 evening: paste prose → ~30s → suggestions queue populates with per-character breakdown. **Remaining for #7**: POV picker UI (so the prompt layer fires at generation time), §10.5 production filters (embedding dedup + evidence-quote validation + prompt-leakage). | 4 | [`LOOM_GENERATION_MODES.md`](LOOM_GENERATION_MODES.md), [`LOOM_STORY_BIBLE.md`](LOOM_STORY_BIBLE.md), [`LOOM_NSFW.md`](LOOM_NSFW.md), [`LOOM_LEDGER_SPIKE.md`](LOOM_LEDGER_SPIKE.md) |

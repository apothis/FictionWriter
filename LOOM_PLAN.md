...
| L5 | Style ingestion (RAG-for-style; chunk reference texts; embed via D StyleDistance + E function-word z-score hybrid index; per-NSFW-status retrieval) | 🚧 production in flight — engine + writer-prompt integration landed 2026-05-13; app-level injection + Bible Workspace surface remaining (see [`HANDOFF.md`](HANDOFF.md) §15.12) | 5 | [`LOOM_RAG_SPIKE.md`](LOOM_RAG_SPIKE.md) §13 (verdict: PIVOT, D+E hybrid); [`LOOM_NARRATIVE_MODE_SPIKE.md`](LOOM_NARRATIVE_MODE_SPIKE.md) §10 (modality classifier); [`LOOM_MLX_PORT_SPIKE.md`](LOOM_MLX_PORT_SPIKE.md) §12 (venv pivot); [`LOOM_RESEARCH.md`](LOOM_RESEARCH.md) §O.4 |
...
| L8 | **Unified Scene Exemplar** — collapse the dual-ingest friction surfaced during Phase 7 live testing (one source body currently has to be added twice: once as a Phase 5 Reference for style-RAG retrieval, once as a Phase 7 Template for structural skeleton). One user action ("Add scene exemplar") fans out to BOTH Pass-A extraction AND chunking+embedding; per-beat writer prompts then retrieve top-K style chunks from the exemplar's own `.index` at generation time so the writer sees structural skeleton + voice descriptor + per-beat-aware retrieved chunks together. Motivated by the NSFW exemplar case where the voice descriptor's 5-field fingerprint under-captures content-register patterns (specific anatomical vocabulary, act-level patterns) that the embedded chunks would carry. Phase 8.a (spike) resolves five open empirical questions: is StyleDistance the right embedder for NSFW register matching vs. mxbai-embed-large / bge-large / E5-mistral-7b; per-beat vs. per-scene retrieval; beat-aware vs. sentence-window chunking; strict vs. softened "do not reuse plot" prompt; full-body-in-prompt vs. retrieval-only. Phase 8.b production: data-model decision (full type merge / shared-storage / rename + soft-merge — design proposes the third), unified ingest action, `BeatGeneration.buildBeatPrompt` extension to accept style exemplars, `TemplateGenerationCoordinator` retrieval wiring, Bible Workspace merged surface. | proposed; design lock pending Phase 8.a spike | 8 | [`LOOM_SCENE_EXEMPLAR.md`](LOOM_SCENE_EXEMPLAR.md) |

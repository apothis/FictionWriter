...
5. **Per-scene-type retrieval pillar.** [LOOM_RESEARCH.md §O.4](LOOM_RESEARCH.md)'s action/dialogue/interiority/description differentiation needs a tagging step at ingest. The spike's [Path C](LOOM_RAG_SPIKE.md) descriptor schema had a `modality` field (action|dialogue|interiority|description) that worked structurally even when the rest collapsed — that schema slot is salvageable for ingest-time scene-type tagging, even though full Path C distillation failed.

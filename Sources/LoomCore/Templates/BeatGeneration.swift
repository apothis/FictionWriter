    ...
    public static func buildBeatPrompt(
        ...
        if includeTemplateBody {
            ...
            if imitateContent {
                systemFraming = """
                    [SYSTEM]
                    You are a fiction writer. Your job is to write ONE beat of a scene whose structural skeleton is given below. Follow the current beat's modality, function, and target word count precisely. Do NOT write the whole scene — only the one beat marked CURRENT. Output ONLY the beat's prose — do NOT emit any prompt-shaped headers like `[VALIDATE BEAT]`, `[BEAT CHECK]`, `[Length: ...]`, `[Pacing: ...]`, or any bracket-wrapped UPPERCASE block. End at a natural sentence boundary without commentary. Use the pronouns specified in the NEW CAST block consistently for each character throughout the beat. Substitute the cast described in the NEW CAST block below; otherwise, preserve the content register, vocabulary, and act patterns that any retrieved style exemplars demonstrate.
                    """
            } else {
                systemFraming = """
                    [SYSTEM]
                    You are a fiction writer. Your job is to write ONE beat of a scene whose structural skeleton is given below. Follow the current beat's modality, function, and target word count precisely. Do NOT write the whole scene — only the one beat marked CURRENT. Output ONLY the beat's prose — do NOT emit any prompt-shaped headers like `[VALIDATE BEAT]`, `[BEAT CHECK]`, `[Length: ...]`, `[Pacing: ...]`, or any bracket-wrapped UPPERCASE block. End at a natural sentence boundary without commentary. Use the pronouns specified in the NEW CAST block consistently for each character throughout the beat. The new scene's content comes from the NEW CAST mapping.
                    """
            }
            templateBlock = ""
        }

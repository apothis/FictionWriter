...
/// On-disk representation for `references/<id>.md`: YAML-ish
/// frontmatter delimited by `---` lines, then prose body. Mirrors
/// `SceneFile` — scalar key:value pairs, double-quoted strings,
/// unknown keys preserved via `extraFrontmatter`. Format:
///
///     ---
///     id: "..."
///     name: "..."
...
public enum ReferenceFile {
    public static func encode(_ ref: ReferenceText) -> String {
        var lines: [String] = ["---"]
        lines.append("id: \(quote(ref.id.uuidString))")
        lines.append("name: \(quote(ref.name))")
        lines.append("nsfw: \(ref.nsfw ? "true" : "false")")
        lines.append("createdAt: \(quote(LoomISO8601.fractionalFormatter.string(from: ref.createdAt)))")
        for key in ref.extraFrontmatter.keys.sorted() {
            let value = ref.extraFrontmatter[key]!
    ...
    public static func decode(_ text: String) throws -> ReferenceText {
        ...
        guard let idStr = fm["id"], let id = UUID(uuidString: idStr) else {
            throw ReferenceFileError.missingOrInvalidID
        }
        let name = fm["name"] ?? ""
    ...
    private static func splitFrontmatterAndBody(_ text: String) throws -> (frontmatter: String, body: String) {
        ...
        guard let closing = closingIndex else {
            throw ReferenceFileError.unterminatedFrontmatter
        }
...
public enum ReferenceFileError: Error, Equatable {

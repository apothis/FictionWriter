...
4. **Writer, unconstrained, single nested object** → small/instruct models couldn't reliably *produce* the deeply-nested skeleton: gemma4_2b free-formed a wrong `title`/`characters`/`plot` shape.
5. **Settled — JSONL.** `BeatExtraction.buildJSONLPrompt` / `parseJSONLSkeleton`: the model emits **one flat JSON object per line** — a `voice` line, one `beat` line each, a `meta` line — the shape the ledger/continuity extractors prove small models hit reliably. Per-line tolerant parse (bad line skipped, unknown enums fall back, `targetWords ≤ 0 → 100`, beats re-indexed; zero beats → `noJSONObjectFound` → retry). Pass-A **prefers the small non-thinking `OllamaBeatExtractor` (gemma4_2b)**, falling back to `KoboldBeatExtractor` (writer, unconstrained, context-sized budget; `useGrammar: true` re-enables the nested-object + GBNF path for a non-thinking writer).

The lessons, in order: **the Ollama `format` schema flakes; GBNF buys well-formed JSON but not well-chosen values; a reasoner's reasoning competes with its output budget; a deeply-nested object is too much for a small model to free-form; and the durable shape is flat JSONL on a small deterministic model.**
